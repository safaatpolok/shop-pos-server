...
public class BranchAnalyticsServiceImpl implements BranchAnalyticsService {
    ...
    public List<PaymentSummary> getPaymentMethodBreakdown(Long branchId, LocalDate date) {
           List<Object[]> rawDate=orderRepository.getPaymentBreakdownByMethod(branchId,date);
           double total = rawDate.stream()
                   .mapToDouble(obj->(Double)obj[1]).sum();

        return rawDate.stream().map(obj->{
    ...
    public BranchDashboardOverviewDTO getBranchOverview(Long branchId) {
        ...
        int yesterdayOrders=orderRepository.countOrdersByBranchAndDate(
                branchId,yesterday
        );
        ...
        // active cashier
           int todayCashiers=orderRepository.countDistinctCashierByBranchAndDate(
                   branchId,today
           );

           int yesterdayCashiers=orderRepository.countDistinctCashierByBranchAndDate(
                   branchId,yesterday
           );

           double cashierGrowth=calculateGrowth(orderGrowth,yesterdayCashiers);

           int todayLowStock = inventoryRepository.countLowerStockItems(branchId);
           int yesterdayLowStock=13;
